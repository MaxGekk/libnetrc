...
class ParserTests extends FlatSpec with Matchers {
  it should "parse the machine item" in {
    ...
    Parsers.parse(item) shouldBe NetRc(Seq(Machine("host1", "root", "123")))
  }
  ...
  it should "parse a multi line machine item" in {
    ...
    Parsers.parse(item) shouldBe NetRc(Seq(
      Machine(m, l, p)
    ))
  }
  ...
  it should "parse the default item" in {
    ...
    Parsers.parse(item) shouldBe NetRc(Seq(Default("root", "123")))
  }
  ...
  it should "parse many items" in {
    val items =
      s"""
         ...
         |""".stripMargin
    Parsers.parse(items) shouldBe NetRc(Seq(
